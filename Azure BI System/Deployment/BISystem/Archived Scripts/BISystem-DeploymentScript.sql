...
GO
PRINT N'Creating [Audit].[RowThroughputCountsCheck]...';
...
CREATE TABLE [Control].[SourceFile] (
    ...
    [FileExtractedParentLoadId] INT            NOT NULL,
...
GO
ALTER TABLE [Audit].[RowThroughputCountsCheck]
    ADD DEFAULT (suser_sname()) FOR [CreatedBy];
...
GO
PRINT N'Creating [Audit].[ObtainRowThroughputCompareS2EandEXT2STG]...';
...
GO
CREATE PROC [Audit].[ObtainRowThroughputCompareS2EandEXT2STG]
(@ProcessStream VARCHAR(10)
)
AS
--DECLARE @ProcessStream VARCHAR(10) = 'EXT2STG'

DECLARE @ProcessExecutionId INT = 0

SELECT @ProcessExecutionId = MAX(ProcessExecutionId)  FROM [Audit].RowThroughputCountsCheck rt
WHERE ProcessStream = @ProcessStream

--Print @ProcessExecutionId
SELECT ISNULL(MIN(CAST([MatchedCounts_YN] AS INT)),0) as MatchedRows FROM [Audit].RowThroughputCountsCheck rt
WHERE ProcessStream = @ProcessStream AND ProcessExecutionId = @ProcessExecutionId
GO
SET ANSI_NULLS, QUOTED_IDENTIFIER ON;


GO
PRINT N'Creating [Audit].[ObtainRowThroughputCountEXT2STGandSTG2CLN]...';
...
GO
CREATE PROC [Audit].[ObtainRowThroughputCountEXT2STGandSTG2CLN]
AS

WITH ParentLoadIdSTG2CLN
AS

(SELECT StageParentLoadId, CleanParentLoadId, SourceFileId, SourceFileName FROM Control.SourceFile WHERE StageDate IS NOT NULL and CleanDate IS NOT NULL And WarehouseDate IS NULL)
, 

LoadIdEXT2STG
AS
(SELECT LoadId FROM [Audit].[PackageLoad] WHERE ParentLoadId IN (SELECT StageParentLoadId FROM ParentLoadIdSTG2CLN))
, 

LoadIdSSTG2CLN
AS
(SELECT LoadId FROM [Audit].[PackageLoad] WHERE ParentLoadId IN (SELECT CleanParentLoadId FROM ParentLoadIdSTG2CLN))
...
INNER JOIN [Control].[SourceFile] sf ON sf.SourceFileId = ce.SourceFileId
	AND sf.SourceFileId IN (SELECT SourceFileId FROM ParentLoadIdEXT2STG)

WHERE ProcessStream = 'EXT2STG'

AND rt.LoadId IN (Select LoadId FROM LoadIdSourceToEXT )
...
INNER JOIN [Control].[SourceFile] sf ON sf.SourceFileId = ce.SourceFileId
		AND sf.SourceFileId IN (SELECT SourceFileId FROM ParentLoadIdSTG2CLN)

WHERE ProcessStream = 'STG2CLN'
AND rt.LoadId IN (Select LoadId FROM LoadIdSourceToEXT )
)
...
AS
(
SELECT sc.ProcessStream, sc.LoadId AS ProcessLoadId, sc.ExecutionId AS ProcessExecutionId, ec.ExtractedRows AS SourceRowCounts, sc.ExtractedRows AS TargetRowcounts
, sc.SourceFileName, sc.EntityName, CASE WHEN ec.InsertedRows = sc.InsertedRows THEN 1 WHEN sc.FailedRows > 0 THEN 1 ELSE 0 END AS MatchedCounts_YN
FROM EXT2STG_Counts ec 
LEFT JOIN STG2CLN_Counts sc ON ec.SourceFileName = sc.SourceFileName
	AND ec.Name = sc.Name
)

--SELECT * FROM CMP_Counts


SELECT ISNULL(MIN(CAST([MatchedCounts_YN] AS INT)), 1) as MatchedRows FROM CMP_Counts rt
WHERE ProcessStream = 'STG2CLN' AND ProcessExecutionId = ProcessExecutionId
GO
SET ANSI_NULLS, QUOTED_IDENTIFIER ON;


GO
PRINT N'Creating [Audit].[ObtainRowThroughputCountS2EandEXT2STG]...';
...
GO
CREATE PROC [Audit].[ObtainRowThroughputCountS2EandEXT2STG]
AS

WITH ParentLoadIdEXT2STG
AS

(SELECT FileExtractedParentLoadId, StageParentLoadId, SourceFileId, SourceFileName FROM Control.SourceFile WHERE StageDate IS NOT NULL and CleanDate IS NULL And WarehouseDate IS NULL)
, 

LoadIdEXT2STG
AS
(SELECT LoadId FROM [Audit].[PackageLoad] WHERE ParentLoadId IN (SELECT StageParentLoadId FROM ParentLoadIdEXT2STG))
, 

LoadIdSourceToEXT
AS
(SELECT LoadId FROM [Audit].[PackageLoad] WHERE ParentLoadId IN (SELECT FileExtractedParentLoadId FROM ParentLoadIdEXT2STG))
, 
EXT2STG_Counts
AS 
(
...
INNER JOIN [Control].EntityType cet ON cet.[Table] = rt.EntityName
INNER JOIN [Control].Entity ce ON ce.[EntityTypeId] = cet.[EntityTypeId]
INNER JOIN [Control].[SourceFile] sf ON sf.[SourceFileId] = ce.[SourceFileId]
	AND rt.SourceFileName = sf.SourceFileName
	AND sf.SourceFileId IN (SELECT SourceFileName FROM ParentLoadIdEXT2STG)
WHERE ProcessStream = 'SRC2EXT'
AND rt.LoadId In (Select LoadId FROM LoadIdEXT2STG )
)
...
AS
(
SELECT sc.ProcessStream, sc.LoadId AS ProcessLoadId, sc.ExecutionId AS ProcessExecutionId, ec.InsertedRows AS SourceRowCounts, sc.InsertedRows AS TargetRowcounts
, sc.SourceFileName, sc.EntityName, CASE WHEN ec.InsertedRows = sc.InsertedRows THEN 1 ELSE 0 END AS MatchedCounts_YN
FROM S2E_Counts ec 
LEFT JOIN EXT2STG_Counts sc ON ec.SourceFileName = sc.SourceFileName
	AND ec.EntityName = sc.Name
...
SELECT ISNULL(MIN(CAST([MatchedCounts_YN] AS INT)), 1) as MatchedRows FROM CMP_Counts rt
WHERE ProcessStream = 'EXT2STG' AND ProcessExecutionId = ProcessExecutionId
GO
SET ANSI_NULLS, QUOTED_IDENTIFIER ON;
