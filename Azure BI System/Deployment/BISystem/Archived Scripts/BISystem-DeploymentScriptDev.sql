...
GO
:setvar DatabaseName "BISystemDev"
:setvar DefaultFilePrefix "BISystemDev"
...
GO
PRINT N'Creating [Config].[GetBlobContainers]...';
...
CREATE PROCEDURE [Config].[GetBlobContainers]  
AS
BEGIN
/*
...
CREATE PROCEDURE [Control].[ObtainDistributionsReadyToProcess]
	@pDistributionName nvarchar(255) OUT
AS
BEGIN
/*
 *	Gets the last inserted connections string for BLOB storage
 */
	SET NOCOUNT ON;
	SET @pDistributionName = (SELECT TOP(1) [SourceFileName] 
							  FROM [Control].[SourceFile] 
							  WHERE [WarehouseDate] IS NULL
							  AND [BlobUploadDate] IS NULL
							  AND [StorSimpleDate] IS NOT NULL
