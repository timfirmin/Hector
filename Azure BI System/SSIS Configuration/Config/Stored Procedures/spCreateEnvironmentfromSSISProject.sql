...
/*
exec [Config].[CreateEnvironmentfromSSISPackage] 'MDSStationIntegration','DEvX','MDS_StationIntegration_SourceToStage'
*/

CREATE PROCEDURE [Config].[spCreateEnvironmentfromSSISProject]
          @folder sysname
        , @env sysname
		, @ObjectName sysname
AS
       ...
       SELECT [env_var] = @tab + @tab + ', @' + ev.name + ' '  + 
	                                           CASE WHEN type in ('nvarchar','String')  THEN  cast('SQL_VARIANT' as varchar) ELSE ev.type  END
