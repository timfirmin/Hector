...
--exec [Config].[ScriptEnvironment] 'BroadcastDataQuality_R1','DevADI'

CREATE PROCEDURE [Config].[spScriptEnvironment]
          @folder sysname
        , @env sysname
AS
        SET NOCOUNT ON;
       ...
       SELECT [env_var] = @tab + @tab + ', @' + ev.name + ' '  + 
	                                           CASE WHEN ev.base_data_type in ('nvarchar')  THEN  cast('SQL_VARIANT' as varchar) ELSE ev.base_data_type  END
