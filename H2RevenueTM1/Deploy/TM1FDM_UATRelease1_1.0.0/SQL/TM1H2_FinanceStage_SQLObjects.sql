...
N'<table style="border:1px dashed black;font-family:arial;">' +
    ...
    CAST ( ( SELECT  
					td = t1.Scenario, '',
					td = '|', '',  
					'td/@bgcolor'=CASE (AlertMe) WHEN 'Y' THEN 'Orange' WHEN 'X' THEN 'White' ELSE 'SpringGreen' END,
...
+ ift.RevenueFileTypeImportFolderName,   @pFilePath
+ ift.RevenueFileTypeProcessedFolderName)
else
replace(ifl.FilePath, @pFilePath
+ ift.RevenueFileTypeImportFolderName,   @pFilePath + ift.RevenueFileTypeArchiveFolderName)
end as ArchivePath
,ift.RevenueFileRetentionDays
,dateadd(day,ift.RevenueFileRetentionDays,CONVERT(DATETIME, ifl.FileCreatedTimeStamp, 103)) as DeleteDateAge
,datediff(day,getdate(),dateadd(day,ift.RevenueFileRetentionDays,CONVERT(DATETIME, ifl.FileCreatedTimeStamp, 103)) ) as DaysUntilDelete
,case when datediff(day,getdate(),dateadd(day,ift.RevenueFileRetentionDays,CONVERT(DATETIME, ifl.FileCreatedTimeStamp, 103)) ) < 1 then 'Y' else 'N' end as DeleteFileFlag
	
 FROM [TM1Control].[ImportFileLog] ifl
 join [TM1Control].[ImportFileType] ift
 on ifl.RevenueFileTypeKey = ift.RevenueFileTypeUniqueKey
