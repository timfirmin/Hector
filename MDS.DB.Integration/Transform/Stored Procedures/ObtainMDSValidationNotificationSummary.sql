...
SET @ReportName = CASE WHEN @TemplateName = 'MDS Validation SDM' THEN 'MDSValidationSDM'
                       WHEN @TemplateName = 'MDS Validation Licencing' THEN 'MDSValidationLicencing'
					   WHEN @TemplateName = 'MDS Validation BI' THEN 'MDSValidationBI' END

SET @NReportLink =  @NReportLink + @ReportName + '?LoadId='+cast (@LoadId as varchar)

;WITH Sample1 AS
 ( SELECT count(V.RuleId) NumberOfValidationFailuers, 
          SUM(CASE WHEN AudioPolicy IS NULL THEN 0 ELSE 1 END) NAudioPolicyUpdates, 
		  SUM(CASE WHEN LiceneGroup IS NULL THEN 0 ELSE 1 END) NLicenceUpdate,
